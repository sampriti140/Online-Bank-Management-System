package Online.Atm;

import java.sql.*;
import javax.swing.*;
import java.awt.*;
import java.awt.event.*;
import java.util.Date;

public class Deposit extends JFrame implements ActionListener {
    JLabel l1, l2;
    TextField t1;
    JButton b1, b2;
    String pin;
    String Accno;

    Deposit(String pin, String Accno) {
        ImageIcon i1 = new ImageIcon(ClassLoader.getSystemResource("icon/atm2.png"));
        Image i2 = i1.getImage().getScaledInstance(1300, 830, Image.SCALE_DEFAULT);
        ImageIcon i3 = new ImageIcon(i2);
        l1 = new JLabel(i3);
        l1.setBounds(0, 0, 1300, 830);
        add(l1);

        this.pin = pin;
        this.Accno = Accno;

        l2 = new JLabel("ENTER YOUR AMOUNT HERE");
        l2.setBounds(410, 180, 400, 35);
        l2.setForeground(Color.WHITE);
        l2.setFont(new Font("system", Font.BOLD, 16));
        l1.add(l2);

        t1 = new TextField();
        t1.setBounds(425, 220, 200, 25);
        t1.setForeground(Color.BLACK);
        t1.setFont(new Font("Raleway", Font.BOLD, 16));
        l1.add(t1);

        b1 = new JButton("DEPOSIT");
        b1.setBounds(570, 364, 150, 30);
        b1.setFont(new Font("Raleway", Font.BOLD, 16));
        b1.addActionListener(this);
        l1.add(b1);

        b2 = new JButton("BACK");
        b2.setBounds(570, 412, 150, 30);
        b2.setFont(new Font("Raleway", Font.BOLD, 16));
        b2.addActionListener(this);
        l1.add(b2);

        setSize(1500, 1080);
        setLocation(0, 0);
        setLayout(null);
        setVisible(true);
    }

    @Override
    public void actionPerformed(ActionEvent e) {
        try {
            String amountStr = t1.getText();
            Date date = new Date();
            if (e.getSource() == b1) {
                if (amountStr.isEmpty()) {
                    JOptionPane.showMessageDialog(null, "Enter your amount");
                } else {
                    double amount = Double.parseDouble(amountStr);

                    // Connect to the database
                    connection c1 = new connection();

                    // Update balance query
                    String updateBalanceQuery = "UPDATE balance SET balance = balance + ? WHERE account_no = ? AND pin = ?";
                    PreparedStatement balanceStmt = c1.getConnection().prepareStatement(updateBalanceQuery);
                    balanceStmt.setDouble(1, amount);
                    balanceStmt.setString(2, Accno);
                    balanceStmt.setString(3, pin);

                    int rowsUpdated = balanceStmt.executeUpdate();

                    if (rowsUpdated > 0) {
                        // Insert transaction record into bank table
                        String insertTransactionQuery = "INSERT INTO bank (pin, date, type, amount, account_no) VALUES (?, ?, ?, ?, ?)";
                        PreparedStatement transactionStmt = c1.getConnection().prepareStatement(insertTransactionQuery);
                        transactionStmt.setString(1, pin);
                        transactionStmt.setTimestamp(2, new Timestamp(date.getTime())); // Use proper date format
                        transactionStmt.setString(3, "deposit");
                        transactionStmt.setDouble(4, amount);
                        transactionStmt.setString(5, Accno);
                        transactionStmt.executeUpdate();

                        JOptionPane.showMessageDialog(null, "Rs. " + amount + " deposited successfully");
                    } else {
                        JOptionPane.showMessageDialog(null, "Failed to update balance. Please check account details.");
                    }

                    // Redirect to the main page
                    new main(pin,Accno);
                    setVisible(false);
                }
            }

            if (e.getSource() == b2) {
                setVisible(false);
                new main(pin,Accno);
            }
        } catch (Exception ex) {
            ex.printStackTrace();
            JOptionPane.showMessageDialog(null, "An error occurred while processing the transaction.");
        }
    }

    public static void main(String[] args) {
        new Deposit("1234", "567890123456");
    }
}
